...
{
    active = state;
    unsigned char modereg = MMA7760_ReadReg(MMA7660_MODE_R);
    modereg &= ~(1<<0);
...
uint8_t MMA7660_GetSingleAxisRaw( uint8_t number )
{
    ...
    do {
        alert = false;
        temp = MMA7760_ReadReg(MMA7660_XOUT_R + number);
        if (temp > 63)
            alert = true;
        if (temp > 31)
            temp += 128+64;
...
void MMA7660_SetSampleRate(uint8_t samplerate)
{
    ...
    temp = MMA7760_ReadReg(MMA7660_SR_R);
...
{
    char tiltreg = MMA7760_ReadReg(MMA7660_TILT_R);
